...
const init = async () => {
  ...
  //inert for insert file into DOM using template reply.file is accessible through inert (file and image etc)
  // Static file and directory handlers for hapi.js.
  await server.register(require("inert"));

  //vision Template rendering support for hapi.js.
  await server.register(require("vision"));

  server.route({
    method: "GET",
    path: "/",
    handler: (req, h) => h.view("index", { name: "Muhammad Moiz" }),
  });
  ...
  server.views({
    engines: {
      //here html is file extensions that we use
